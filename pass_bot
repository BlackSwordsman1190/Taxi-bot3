...
order_message += f"\n🔗 Waze Navigation: {context.user_data['waze_link']}\n"
...
await query.edit_message_text(
    "✅ Your order has been accepted, wait for a call from the driver."
)
...
await update.message.reply_text(
    "Order cancelled. Press the button to start a new order.",
    reply_markup=reply_markup
)
return ConversationHandler.END
```
...
try:
    ...
    if chat_id not in driver_chat_ids:
        ...
        await update.message.reply_text(f"✅ Driver {chat_id} added successfully!\nTotal drivers: {len(driver_chat_ids)}")
    else:
...
try:
    ...
    if chat_id in driver_chat_ids:
        ...
        await update.message.reply_text(f"✅ Driver {chat_id} removed successfully!\nTotal drivers: {len(driver_chat_ids)}")
    else:
